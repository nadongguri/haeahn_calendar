...
- 모바일에서는 월간 달력을 기본으로 표시합니다. `새 예약` 버튼 또는 달력의 날짜를 누르면 예약 입력창이 열립니다.
- `월` / `주` / `일` 보기로 전환할 수 있습니다. 주간 보기만 토·일을 제외하며, 일간 보기에서는 주말도 조회하고 예약할 수 있습니다.
- 화면 상단에 오늘 날짜를 표시하고 달력 안의 오늘 날짜를 파란색으로 강조합니다. 주·일 보기 머리글은 첫 줄에 날짜, 둘째 줄에 요일을 표시합니다.
...
The attendee field and notification checkbox are hidden in the modal, and saved reservations store `send_notification = false`.
